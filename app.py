...
/* Selectbox — no blur on this one to prevent pixellation */
...
[data-testid="stButton"] > button {
  ...
  white-space:nowrap !important; padding:0 1.5rem !important;
}
...
/* ---- SEARCH CARD: style Streamlit's own column container ---- */
/* Target the block that wraps the columns — make it look like a card */
[data-testid="stMainBlockContainer"] > div > div > div:nth-child(3) {
...
/* Search title bar — merges visually with the column card below */
.ss-head { max-width:960px; margin:-1px auto 0; padding:0 2rem; position:relative; z-index:10; }
...
.ss-ttl-text { font-family:'Syne',sans-serif; font-size:1.05rem; font-weight:700; color:#0F172A; letter-spacing:-.01em; }
.ss-ttl-text span { color:#6366F1; }

/* Loading */
.sl-wrap { max-width:960px; margin:1rem auto 0; padding:0 2rem 2rem; }
...
/* Results — gradient cards */
.sr-wrap { max-width:960px; margin:1.5rem auto 0; padding:0 2rem 4rem; }
...
.sr-rb.mo { background:linear-gradient(135deg,#FEF3C7,#FDE68A22); border:1.5px solid #FCD34D; border-bottom:none; }
...
.sr-rb.st .sr-rt { color:#065F46; }
.sr-rb.mo .sr-rt { color:#92400E; }
.sr-rb.wk .sr-rt { color:#991B1B; }

/* Main result card — gradient tint */
.sr-mc {
  background:linear-gradient(135deg,#FAFBFF 0%,#F5F7FF 50%,#F8F6FF 100%);
  border-radius:0 0 16px 16px;
  box-shadow:0 8px 40px rgba(67,56,202,0.12),0 2px 6px rgba(0,0,0,0.05);
  border:1.5px solid #E2E8F0; border-top:none;
  padding:2.2rem 2.5rem; margin-bottom:1rem;
...
/* Explanation card — gradient tint */
.sr-ec {
...
.sr-fbb { display:flex; gap:.5rem; }
.sr-fbt { background:#F8FAFC; border:1.5px solid #E2E8F0; border-radius:9px; padding:.45rem 1.1rem; font-family:'DM Sans',sans-serif; font-size:.84rem; font-weight:500; color:#475569; cursor:pointer; transition:all .18s; }
.sr-fbt:hover { border-color:#6366F1; color:#6366F1; background:rgba(99,102,241,.06); }
...
/* Feedback buttons — don't stretch full width */
.sr-fbc + div [data-testid="stButton"] > button {
...
.sr-fbc + div [data-testid="stButton"] > button:hover {
  border-color: #6366F1 !important;
  ...
  <div class="sh-i">
    ...
    <div class="sh-r">
      <div class="sh-stats">
        ...
        <div class="sh-stat"><div class="sh-sn">78%</div><div class="sh-sl">Max Confidence</div></div>
      </div>
    </div>
  </div>
</div>
...
# ── SEARCH — title merges with column card ────────────────────────────
st.markdown("""
<div class="ss-head">
  <div class="ss-ttl">
...
# Columns sit inside .stHorizontalBlock which is styled as the bottom of the card
col1, col2, col3 = st.columns([3, 2, 1.4])
with col1:
    company_input = st.text_input("Company", placeholder="e.g. Google, Amazon, Microsoft", key="ci")
with col2:
    role_input = st.selectbox("Target Role", options=list(ROLE_SOC_MAP.keys()), index=0, key="ri")
with col3:
...
if calculate:
    ...
    if not company:
        st.markdown('<div class="sr-wrap"><div class="sr-err"><h3>Enter a company name</h3><p>Try "Google", "Amazon", or "Microsoft".</p></div></div>', unsafe_allow_html=True)
    else:
...
""", unsafe_allow_html=True)
        ...
        # Feedback
        st.markdown("""<div class="sr-fbc"><div class="sr-fbl">Was this score accurate for your experience?</div></div>""", unsafe_allow_html=True)
        fb_col1, fb_col2, fb_col3 = st.columns([2, 2, 8])
        with fb_col1:
        ...
        if fb == "positive":
            st.markdown("""<div style="max-width:960px;margin:0 auto;padding:0 2rem;"><p style="font-family:DM Sans,sans-serif;font-size:.85rem;color:#059669;font-weight:500;">Thanks for the feedback! Glad it was helpful.</p></div>""", unsafe_allow_html=True)
        elif fb == "negative":
            st.markdown("""<div style="max-width:960px;margin:0 auto;padding:0 2rem;"><p style="font-family:DM Sans,sans-serif;font-size:.85rem;color:#D97706;font-weight:500;">Thanks for the feedback. We will keep improving the scores.</p></div>""", unsafe_allow_html=True)

        st.markdown("""<div class="sr-foot">Data: USCIS H1B Employer Data Hub + DOL LCA Disclosure Files · FY2021–FY2026 · Historical patterns only, not a guarantee of future intent</div>""" , unsafe_allow_html=True)